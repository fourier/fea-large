function [m,p] = boundary_conditions(M,P,boundary,symmetry)
...
for i = 1:boundary_count
	% if it is "fake" condtion(0) or y-condition only - skip
	if boundary(i,4) == 0 || boundary(i,4) == 2
		continue
	end	
	index = boundary(i,1)*2-1; % x-coordinate
	tmp = m(index,index);
...
for i = 1:boundary_count
	% if it is "fake" condtion(0) or x-condition only - skip
	if boundary(i,4) == 0 || boundary(i,4) == 1
		continue
	end	
	index = boundary(i,1)*2; % y-coordinate
	tmp = m(index,index);
...
for i=1:sym_count
	index = symmetry(i)*2-1; % x-coordinate
	tmp = m(index,index);
	for j=1:msize
		m(j,index) = 0;
		m(index,j) = 0;
