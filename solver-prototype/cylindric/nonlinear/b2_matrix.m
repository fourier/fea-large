...
% by given element and point(r,z)
  ...
  % row1 = \sum\limits^{12}_{j=1}H^{ji}_{11}q_j
	...
	for i = 1:12	
		for j = 1:12
			B(1,i) = B(1,i) + 0.5 * ( L(1,i)*L(1,j)+L(4,i)*L(4,j) )*q(j);
			B(2,i) = B(2,i) + 0.5 * ( L(5,i)*L(5,j)+L(2,i)*L(2,j) )*q(j);
			B(3,i) = B(3,i) + 0.5 * L(3,i)*L(3,j)*q(j);
			B(4,i) = B(4,i) + 0.5 * ( L(1,i)*L(5,j)+L(4,i)*L(2,j) )*q(j);
		end
	end
